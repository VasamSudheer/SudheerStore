    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult LogOn(LogOnModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                bool checkUser=db.UserData.Any(u => u.UserName == model.UserName & u.Password==model.Password);

                if (checkUser)
                {
                    Session["UserDetails"] = db.UserData
                                             .Where(u => u.UserName == model.UserName & u.Password == model.Password)
                                            .FirstOrDefault();
                    Session["UserID"] = db.UserData
                                             .Where(u => u.UserName == model.UserName & u.Password == model.Password)
                                            .FirstOrDefault();

                    var sss = Convert.ToString(Session["UserName"]);
